...
const views = {
  posts: { eyebrow: "投稿管理", title: "投稿予定リスト", dateKey: "postedAt" },
...
function parseGeneratedCopies(value) {
  const sections = { Instagram: "", X: "", "LINE配信": "" };
...
function createCopyButton(target, label) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "copy-button";
  button.textContent = "コピー";
  button.setAttribute("aria-label", `${label}をコピー`);
  button.addEventListener("click", () => copyText(target.value).catch(() => showCopyStatus("コピーに失敗しました")));
  return button;
}

function createGeneratedCopiesField(values) {
  const wrapper = document.createElement("div");
  wrapper.className = "copy-output generated-copy-panel full-span";

  const title = document.createElement("div");
  title.className = "copy-field-title";
  const titleText = document.createElement("span");
  titleText.textContent = fieldLabels.generatedCopies;
  const badge = document.createElement("span");
  badge.className = "auto-badge";
  badge.textContent = "自動作成";
  title.append(titleText, badge);

  const hidden = document.createElement("textarea");
  hidden.name = "generatedCopies";
  hidden.value = values.generatedCopies ?? "";
  hidden.hidden = true;

  const sections = parseGeneratedCopies(hidden.value);
  const sectionInputs = [
    ...
    sectionTitle.className = "copy-section-title";
    const sectionLabel = document.createElement("strong");
    sectionLabel.textContent = label;

    const textarea = document.createElement("textarea");
    textarea.dataset.copyPart = key;
    textarea.value = sections[key];
    textarea.rows = key === "LINE配信" ? 9 : 7;

    sectionTitle.append(sectionLabel, createCopyButton(textarea, label));
    section.append(sectionTitle, textarea);
    wrapper.appendChild(section);
    return textarea;
  });

  const syncHidden = () => {
    const nextSections = Object.fromEntries(sectionInputs.map((textarea) => [textarea.dataset.copyPart, textarea.value]));
    hidden.value = buildGeneratedCopiesFromSections(nextSections);
    hidden.dataset.autoGenerated = "false";
  };

  sectionInputs.forEach((textarea) => textarea.addEventListener("input", syncHidden));
  wrapper.prepend(title, hidden);
  return wrapper;
}
...
function createField(field, values, config) {
  ...
  if (["canvaHeadline", "canvaSubcopy", "canvaNote"].includes(field)) {
    ...
    title.className = "copy-field-title";
    const titleText = document.createElement("span");
    titleText.textContent = fieldLabels[field];
    const rightControls = document.createElement("span");
    rightControls.className = "copy-section-title";
    const badge = document.createElement("span");
    badge.className = "auto-badge";
    badge.textContent = "自動作成";
    rightControls.append(badge, createCopyButton(input, fieldLabels[field]));
    title.append(titleText, rightControls);
    label.textContent = "";
...
function setGeneratedCopiesValue(value) {
  const generatedCopiesInput = entryForm.elements.generatedCopies;
  if (!generatedCopiesInput) return;
  generatedCopiesInput.value = value;
  const sections = parseGeneratedCopies(value);
  entryForm.querySelectorAll("textarea[data-copy-part]").forEach((textarea) => {
    textarea.value = sections[textarea.dataset.copyPart] || "";
  });
}
...
function syncPostCopy({ force = false } = {}) {
  ...
  autoTargets.forEach(([input, value]) => {
    ...
    if (force || !input.value.trim() || input.dataset.autoGenerated === "true") {
      if (input.name === "generatedCopies") setGeneratedCopiesValue(value);
...
function currentFormAreas() {
  const checkedAreas = uniqueAreas([...entryForm.querySelectorAll('input[name="serviceAreaOptions"]:checked')].map((checkbox) => checkbox.value));
  if (checkedAreas.length > 0) return checkedAreas;
  return uniqueAreas(areaPresets.sapporoOtaru.areas);
}

function createWeeklyPosts() {
  const baseDate = entryForm.elements.postedAt?.value || todayISO();
  const selectedAreas = currentFormAreas();
  const serviceAreas = selectedAreas.join("、");
  const weeklyPosts = weeklyPatternOrder.map((patternKey, index) => {
    const pattern = selectedAppealPattern(patternKey);
    const postSet = createPostSet(patternKey, selectedAreas);
    const postedAt = addDaysISO(baseDate, index);
    ...
    return normalizeRecord({
      ...
      generatedCopies: createGeneratedCopies(patternKey, selectedAreas),
      canvaHeadline: postSet.canvaHeadline,
      canvaSubcopy: postSet.canvaSubcopy,
      canvaNote: postSet.canvaNote,
      recordType: "posts",
      memo: `7日分まとめて作成：${index + 1}日目（${pattern.label}）`
    });
  });

  records = [...weeklyPosts, ...records];
  saveRecords();
  editingId = null;
  currentView = "posts";
...
function renderPostScheduleHeading(data) {
  if (!postScheduleList) return;
  postScheduleList.hidden = currentView !== "posts";
  if (currentView !== "posts") return;

  const label = currentMonth === "all" ? "すべての月" : `${currentMonth.replace("-", "年")}月`;
  const note = postScheduleList.querySelector(".schedule-note");
...
function createStatusButton(status, label, activeStatus) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "ghost-button status-action-button";
  button.dataset.status = status;
  button.textContent = label;
  button.setAttribute("aria-pressed", String(activeStatus === status));
  if (activeStatus === status) button.classList.add("active-status");
  return button;
}
...
function createPostManagementControls(record) {
  ...
  statusControls.className = "status-button-group";
  statusControls.append(
    createStatusButton("反応あり", "反応ありにする", record.postStatus),
    createStatusButton("問い合わせあり", "問い合わせありにする", record.postStatus)
  );
  ...
  memoInput.rows = 2;
  memoInput.setAttribute("aria-label", `${record.title || "投稿"}の反応メモ`);
  ...
  countInput.setAttribute("aria-label", `${record.title || "投稿"}の問い合わせ件数`);
  countLabel.appendChild(countInput);

  wrapper.append(statusControls, memoLabel, countLabel);
  return wrapper;
}
...
function renderList() {
  ...
  data.forEach((record) => {
    ...
    if (currentView === "posts") {
      ...
      markPostedButton.hidden = record.postStatus === "投稿済み";
    } else {
...
if (bulkCreatePosts) bulkCreatePosts.addEventListener("click", createWeeklyPosts);
...
list.addEventListener("click", (event) => {
  ...
  if (event.target.classList.contains("status-action-button")) {
    const nextStatus = normalizePostStatus(event.target.dataset.status);
    records = records.map((item) => (item.id === record.id ? normalizeRecord({ ...item, postStatus: nextStatus }) : item));
    saveRecords();
    refreshScreen();
    showCopyStatus(`投稿ステータスを${nextStatus}にしました`);
    return;
  }
