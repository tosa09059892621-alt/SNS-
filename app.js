const storageKey = "snsSidejobRecords";
...
const fieldIds = ["recordId", "postedAt", "media", "title", "body", "canvaUrl", "postStatus", "inquiryAt", "inquiry", "workDate", "workDetail", "sales", "cost", "responseStatus", "memo"];
const fields = Object.fromEntries(fieldIds.map((id) => [id, document.querySelector(`#${id}`)]));
...
function normalizeRecord(record) {
  return {
    ...
    postedAt: record.postedAt || new Date().toISOString().slice(0, 10),
    media: record.media || "Instagram",
    title: record.title || "無題の投稿",
    body: record.body || "",
    canvaUrl: record.canvaUrl || "",
    postStatus: record.postStatus || "下書き",
    inquiryAt: record.inquiryAt || record.postedAt || new Date().toISOString().slice(0, 10),
    inquiry: record.inquiry || "",
    workDate: record.workDate || record.postedAt || new Date().toISOString().slice(0, 10),
    workDetail: record.workDetail || "",
    sales: Number(record.sales) || 0,
    cost: Number(record.cost) || 0,
    responseStatus: record.responseStatus || "未対応",
    memo: record.memo || ""
  };
}
...
function loadRecords() {
  ...
  try {
    ...
    console.warn("保存データの読み込みに失敗しました。サンプルデータを表示します。", error);
  }
...
function setupMonthFilter() {
  ...
  currentMonth = selectedMonth !== "all" && months.includes(selectedMonth) ? selectedMonth : "all";
...
function metaFor(record) {
  if (currentView === "posts") {
    return [
      ...
      ["Canva画像リンク", record.canvaUrl || "未入力"],
...
function textFor(record) {
  if (currentView === "inquiries") return { title: record.inquiry || "問い合わせ内容未入力", body: record.body, date: record.inquiryAt, pill: record.responseStatus };
...
function renderList() {
  ...
  if (data.length === 0) {
    list.innerHTML = '<p class="empty-state">この月のデータはありません。入力フォームから追加できます。</p>';
    return;
  }
  ...
  data.forEach((record) => {
    ...
    node.querySelector(".edit-button").addEventListener("click", () => editRecord(record.id));
...
function readFormRecord() {
  return normalizeRecord({
    ...
    postedAt: fields.postedAt.value,
    media: fields.media.value,
    title: fields.title.value.trim(),
    body: fields.body.value.trim(),
    canvaUrl: fields.canvaUrl.value.trim(),
    postStatus: fields.postStatus.value,
    inquiryAt: fields.inquiryAt.value,
    inquiry: fields.inquiry.value.trim(),
    workDate: fields.workDate.value,
    workDetail: fields.workDetail.value.trim(),
    sales: fields.sales.value,
    cost: fields.cost.value,
    responseStatus: fields.responseStatus.value,
...
function fillForm(record) {
  ...
  fields.postedAt.value = record.postedAt;
  fields.media.value = record.media;
  fields.title.value = record.title;
  fields.body.value = record.body;
  fields.canvaUrl.value = record.canvaUrl;
  fields.postStatus.value = record.postStatus;
  fields.inquiryAt.value = record.inquiryAt;
  fields.inquiry.value = record.inquiry;
  fields.workDate.value = record.workDate;
  fields.workDetail.value = record.workDetail;
  fields.sales.value = record.sales;
  fields.cost.value = record.cost;
  fields.responseStatus.value = record.responseStatus;
...
function resetForm() {
  ...
  fields.postedAt.value = new Date().toISOString().slice(0, 10);
  fields.inquiryAt.value = fields.postedAt.value;
  fields.workDate.value = fields.postedAt.value;
  fields.sales.value = 0;
  fields.cost.value = 0;
  formMode.textContent = "新規追加";
}
...
function deleteRecord(id) {
  const record = records.find((item) => item.id === id);
  if (!record) return;
  const confirmed = confirm(`「${record.title}」を削除しますか？`);
  if (!confirmed) return;
...
function exportCsv() {
  ...
  anchor.href = url;
  anchor.download = "sns-sidejob-data.csv";
  anchor.click();
...
form.addEventListener("submit", saveFormRecord);
